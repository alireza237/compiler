...
void ClassTable::construct_class_table() {
    ...
    for (int i = this->class_list->first(); this->class_list->more(i); i = this->class_list->next(i)) {
        ...
        if (this->class_table->lookup(class_name_symbol) != NULL) {
            semant_error(cls);
            this->error_stream << " CRITICAL: Class \'" << class_name_symbol->get_string() << "\' redefinition."
                               << endl;
            continue;
        }
        ...
        if (parent_name_symbol == Int || parent_name_symbol == Bool || parent_name_symbol == Str ||
            parent_name_symbol == SELF_TYPE) {
            semant_error(cls);
            this->error_stream << " CRITICAL: Class \'" << class_name_symbol->get_string()
                               << "\' inherits from Int, Bool, Str or SELF_TYPE." << endl;
            continue;
        }
...
bool ClassTable::check_parents() {
    ...
    for (int i = this->class_list->first(); this->class_list->more(i); i = this->class_list->next(i)) {
        ...
        if (this->class_table->lookup(parent_name_symbol) == NULL && parent_name_symbol != No_class) {
            semant_error(cls);
            this->error_stream << " CRITICAL: Parent class \'" << cls->get_name_symbol()->get_string()
                               << "\' of class \'" << parent_name_symbol->get_string() << "\' not defined!" << endl;
            ret_val = false;
        }
    }
...
/*
map < Symbol , vector<Symbol> > adj ;
void build_graph(map <Symbol, Symbol> g){
    for(map < Symbol , Symbol > :: iterator it = g.begin() ; it != graph.end()){
        Symbol a = (*it).first;
        Symbol b =  (*it).second;
        adj[b].push_back(a);
    }
}
map<pair<Symbol , int> , Symbol> parr ;
map<Symbol , int> height ;
void dfs(Symbol v , map <Symbol, Symbol> g){
    if(v == Object)
        parr[make_pair(v , 0)] = Object ;
    else
        parr[make_pair(v , 0)] = g[v];

    for(int i  = 1 ; i < 10 ; i ++ ){
        parr[make_pair(v , i)] = parr[make_pair(parr[make_pair(v , i - 1)] , i - 1)];
    }
    for(auto u : adj[v]){
        if(u == g[v]) continue;
        height[u] = height[v] + 1;
...
Feature class__class::get_attr(char *feature_name) {
    ...
    for (int i = features->first(); features->more(i); i = features->next(i)) {
        f = features->nth(i);
        if (f->get_formals() == NULL &&
            strcmp(const_cast<char *>(f->get_name_symbol()->get_string()), feature_name) == 0) {
            return f;
        }
    }
    return NULL;
...
void method_class::check() {
    ...
    if (classtable->class_table->lookup(return_type) == NULL && return_type != SELF_TYPE) {
        ...
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "Unknown return type " << return_type << std::endl;
    ...
    if (feature != NULL) {
        ...
        for (int i = formals->first(); formals->more(i); i = formals->next(i)) {
            ...
            if (f->get_formal_type() != parent_f->get_formal_type()) {
                ...
                classtable->semant_error(curr_class->get_filename(), this) <<
                                                                           ...
                                                                           << parent_f->get_formal_type()
                                                                           << std::endl;

            }
            if (tof == parent_formals->len())
                break;
        }
        ...
        if (feature->get_type() != return_type) {
            //throw "formal type mismatch";
            classtable->semant_error(curr_class->get_filename(), this) <<
                                                                       "In redefined metod " << name << " ,return type "
                                                                       << return_type
                                                                       << " is diffresnt from orginal return type " <<
                                                                       feature->get_type()
                                                                       << std::endl;
    ...
    if (g->conform(expr->get_type(), return_type) == false) {
        ...
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   ...
                                                                   << " of method init does not conform "
                                                                      "to declared return type " << return_type << "."
                                                                   << std::endl;
        return;
...
void attr_class::check() {
    ...
    if (symboltable->probe(const_cast<char *>(name->get_string())) != NULL) {
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "Attribute " << name
                                                                   << " is already defined either in the same class "
    ...
    while (true) {
        ...
        if (feature != NULL) {
            ...
            classtable->semant_error(curr_class->get_filename(), this) <<
                                                                       "Attribute " << feature->get_name_symbol()
                                                                       << " is already defined "
                                                                          "either in  in a superclass."
                                                                       << std::endl;


            type_decl = feature->get_type();
            //return;
        }
...
void formal_class::check() {
    ...
    target_class = classtable->class_table->lookup(type_decl);
...
void branch_class::check() {
    if (type_decl == SELF_TYPE) {
        ...
        //throw "Identifier declared with type SELF_TYPE in case branch";
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "Identifier declared with type SELF_TYPE in case branch"
                                                                   << std::endl;
    }
    if (classtable->class_table->lookup(type_decl) == NULL) {
        //throw "Undefined class of case branch";
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "Undefined class of case branch"
                                                                     << std::endl;
    ...
    if (g->conform(exprtype, mytype) == false) {
        ...
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   ...
                                                                   << " of assigned expression does not conform to "
                                                                      "declared type " << mytype << " of identifier "
                                                                   << name << "." << std::endl;
        //type = Object;
        //return;
    }
...
void static_dispatch_class::check() {
    ...
    if (g->conform(expr_type, type_name) == false) {
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   ...
                                                                   << " does not conform to declared static "
                                                                      "dispatch type " << type_name << "." << std::endl;
        //return;
    }
    ...
    if ( actual->more(i) or formals_type->more(i)) {
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "Method " << name << " called with wrong number"
...
void let_class::check() {
    ...
    if (init_type != No_type && g->conform(init_type, type_decl) == false) {
        ...
        "Inferred type " << init_type << " of initialization of " << identifier <<
                         " does not conform to identifier's declared type " << type_decl << "." << std::endl;
    }
...
void plus_class::check() {
    if (!check_arith(e1, e2)) {
        ...
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "non-Int arguments: " << e1->get_type() << " + " << e2->get_type() << std::endl;
...
void neg_class::check() {
    if (!check_arith(e1)) {
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "non-Int arguments: " << e1->get_type() <<" ~ " << std::endl;
    }
}
...
void comp_class::check() {
    if (!check_comp(e1)) {
        ...
        classtable->semant_error(curr_class->get_filename(), this) <<
                                                                   "non-Bool argument: " << e1->get_type() << " --not-- "  << std::endl;
    }
}
...
void new__class::check() {
    ...
    if(classtable->class_table->lookup(new_type)==NULL)
        // throw "undefined class in new";
        classtable->semant_error(curr_class->get_filename(), this) << "undefined type in new"<<std::endl;
...
void object_class::check() {
    ...
    if (mytype == NULL) {
        //throw "type error in object_class";
        classtable->semant_error(curr_class->get_filename(), this) << "type error in object class"<<std::endl;
    }
    type = mytype;
}
